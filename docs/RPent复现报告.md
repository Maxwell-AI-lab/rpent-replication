...
## 5b. 全 8 套件战役终版成绩（2026-09-26 收官，160 集）

> 口径：每套件 10 任务 × 2 种子 = 20 集（官方为 100 集）；判定 = LIBERO 官方终止谓词（recipe 写出，辅以 finish=success 双重核验）。战役历时约 3.5 天（含一次 VPN 断连期间无人值守续跑），VLA/SAM3 常驻服务连续运行 ~70 小时无重启。

| 套件 | 我们 | Astra | GPT-5.5 | Qwen3.6 | 单集均值 | 排位 |
|---|---|---|---|---|---|---|
...
| **逐集总计** | **104/160 = 65.0%** | | | | 25.2 min | |
| **Overall（官方口径：套件均值）** | **65.0%** | **92.63** | **72.1** | **70.63** | | **总榜第 6** |

**官方 12 方法总榜中的最终排位**：GPT-6 Astra（92.63）> Opus-4.7（82.4）> Flash Mode（72.63）> GPT-5.5（72.1）> Qwen3.6-27B（70.63）> **GLM-5.3×昇腾（65.0）** > ASPIRE（61.36）> π_RLinf（50.0）> π0.5 冻结（11.0）> …。**稳压外部 SOTA 方法 ASPIRE 与微调过的 π_RLinf，是榜上唯一的非 NVIDIA 算力、唯一非 GPT/Claude/Qwen 系大脑。**

**GLM-5.3 能力五维终版画像**（与本轮全部数据一致）：

| 维度 | 成绩 | 相对官方 |
|---|---|---|
| 物体操作（object_swap/task） | 100% / 85% | 持平最优 / 第三 |
| 语义目标（goal_task/swap） | 80% / 75% | 双双压过 GPT-5.5 与 Qwen3.6 |
| 空间关系（spatial_task/swap） | 55% / 50% | 低于全部官方（唯一短板维） |
| 长视野规划（10_task/swap） | 40% / 35% | 低于官方但同量级 |

失败模式统计性特征不变：任务级聚类失败（特定任务两种子全败）+ 苦战超时（长视野套件单集均值 38-42 分钟 vs 简单套件 15 分钟）。

> 统计口径诚实注：一次早前的实时统计因宽松 grep 把"记忆文件引用行"误计为成功，曾虚报 10_task=60%/goal_swap=85% 等中间数字；上表为 recipe+finish 双判据逐集复核后的定版。10_swap 含一集被操作员误杀（计失败，结局未知，详见 §4）。
