...
每轮一行：**主责模块**（该轮墙钟时间的大头）、上下文/缓存/prefill/生成、LLM 耗时（**拆解为 TTFT + 生成**）、决策内容、工具调用。

| 轮 | 时刻 | 主责模块 | 上下文 tok | 缓存命中 | 新增 prefill | 生成 tok | LLM 耗时 s | TTFT≈s | 生成≈s | 输出内容（决策摘要） | 工具调用（耗时） | 工具耗时 s |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 1 | 03:26:15 | Agent·记忆 | 12,779 | 0 | 12779 | 144 | 0.0 | 0 | 3.2 | 读记忆层与指南（开局固定动作） | list_dir(0s) read_text_file(0s) read_text_file(0s) list_dir(0s) | 0 |
| 2 | 03:26:27 | Agent·记忆 | 19,323 | 12,736 | 6587 | 547 | 12.0 | 0（decode 主导） | 12.0 | 任务记忆缺失，读套件叶子卡片 | read_text_file(0s) read_text_file(0s) read_text_file(0s) read_text_file(0s) | 0 |
| 3 | 03:26:37 | Agent·记忆 | 35,722 | 19,264 | 16458 | 385 | 10.0 | 2（prefill+排队） | 8.5 | 读同 regime 兄弟套件策略 | read_text_file(0s) read_text_file(0s) read_text_file(0s) | 0 |
| 4 | 03:26:48 | Agent·记忆 | 46,669 | 35,712 | 10957 | 532 | 11.0 | 0（decode 主导） | 11.7 | 掌握技巧：tomato-can→basket 模式，OBJECT 坐标系 | read_text_file(0s) read_text_file(0s) read_text_file(0s) read_text_file(0s) | 0 |
| 5 | 03:27:01 | GLM·看图 | 48,768 | 46,656 | 2112 | 455 | 13.0 | 3（prefill+排队） | 10.0 | 解析任务语言：抓沙拉酱瓶放入筐（与记忆不同，需现场定位） | read_image(0s) | 0 |
| 6 | 03:27:13 | GLM·诊断 | 49,239 | 48,704 | 535 | 177 | 12.0 | 8（prefill+排队） | 3.9 | read_image 只返回元数据（图片未入多模态），改用 SAM3 | read_image(0s) | 0 |
| 7 | 03:28:15 | SAM3 | 49,431 | 49,216 | 215 | 2468 | 62.0 | 8（prefill+排队） | 54.3 | 决定并行分割 4 个物体（本轮思考 2.4k tok） | segment(1s) segment(0s) segment(0s) segment(0s) | 1 |
| 8 | 03:28:57 | SAM3 | 52,627 | 48,704 | 3923 | 1604 | 37.0 | 2（prefill+排队） | 35.3 | SAM3 全部命中：沙拉酱 (-0.184,-0.080) 与相近瓶区分成功 | segment(2s) segment(0s) segment(0s) segment(0s) segment(0s) | 2 |
| 9 | 03:29:46 | Agent·反投影 | 55,151 | 52,608 | 2543 | 1833 | 43.0 | 3（prefill+排队） | 40.3 | 交叉验证定位表：红挤瓶/高玻璃瓶/篮筐坐标确认 | back_project(0s) back_project(0s) back_project(0s) back_project(0s) | 0 |
| 10 | 03:30:40 | env·移动 | 57,408 | 55,104 | 2304 | 2058 | 54.0 | 9（prefill+排队） | 45.3 | 定位表完成，move_to 预定位至抓取点上方 | move_to(14s) | 14 |
| 11 | 03:31:12 | SAM3 | 60,124 | 48,768 | 11356 | 125 | 18.0 | 15（prefill+排队） | 2.8 | 预定位成功（Δ=8.8mm），决定腕部精修 | segment(1s) | 1 |
| 12 | 03:31:30 | π0.5·抓取 | 60,433 | 55,168 | 5265 | 491 | 17.0 | 6（prefill+排队） | 10.8 | 腕部精修通过（4mm 内），锁定最终抓取目标 | pi0_pick(48s) | 48 |
| 13 | 03:32:48 | SAM3·验证 | 61,666 | 55,168 | 6498 | 541 | 30.0 | 18（prefill+排队） | 11.9 | pi0_pick 执行后 terminated=true（峰值抬升 0.196m） | segment(2s) | 2 |
| 14 | 03:33:25 | Agent·审计 | 62,395 | 55,168 | 7227 | 1785 | 35.0 | 0（decode 主导） | 39.3 | 验证：瓶子已在篮筐区 (-0.005,0.264)，写审计 | write_text_file(0s) | 0 |
| 15 | 03:33:41 | Agent·收官 | 64,221 | 62,336 | 1885 | 223 | 16.0 | 11（prefill+排队） | 4.9 | 审计完成，finish(success) 收官 | finish(0s) | 0 |

**口径说明**：TPOT≈**22 ms/tok**（由 2/5/7/8/9/10 六个高生成轮拟合，如轮 7：2468 tok / 62s）；生成耗时 = 生成 tok × TPOT；**TTFT = 响应耗时 − 生成耗时**（含 prefill、排队、HK→bigmodel 网络往返，估算法精度 ±3s）。TTFT=0 的轮次为 decode 主导；本环境 TTFT 固定开销约 **8-12 秒**——这就是为什么即使缓存命中 99%、生成仅百余 token 的轮次也要 12-18 秒。秒级日志粒度下亚秒级正文尾巴并入思考生成段。
