...
每轮一行：上下文长度、缓存命中、新增 prefill、生成 token、LLM 耗时、**输出内容（决策摘要）**、本轮工具调用。

| 轮 | 时刻 | 上下文 tok | 缓存命中 | 新增 prefill | 生成 tok | LLM 耗时 s | 输出内容（决策摘要） | 本轮工具调用（耗时） |
|---|---|---|---|---|---|---|---|---|
| 1 | 03:26:15 | 12,779 | 0 | 12,779 | 144 | 0 | 读记忆层与指南（开局固定动作） | list_dir(0s) read_text_file(0s) read_text_file(0s) list_dir(0s) |
| 2 | 03:26:27 | 19,323 | 12,736 | 6,587 | 547 | 12 | 任务记忆缺失，读套件叶子卡片 | read_text_file(0s) read_text_file(0s) read_text_file(0s) read_text_file(0s) |
| 3 | 03:26:37 | 35,722 | 19,264 | 16,458 | 385 | 10 | 读同 regime 兄弟套件策略 | read_text_file(0s) read_text_file(0s) read_text_file(0s) |
| 4 | 03:26:48 | 46,669 | 35,712 | 10,957 | 532 | 11 | 掌握技巧：tomato-can→basket 模式，OBJECT 坐标系 | read_text_file(0s) read_text_file(0s) read_text_file(0s) read_text_file(0s) |
| 5 | 03:27:01 | 48,768 | 46,656 | 2,112 | 455 | 13 | 解析任务语言：抓沙拉酱瓶放入筐（与记忆不同，需现场定位） | read_image(0s) |
| 6 | 03:27:13 | 49,239 | 48,704 | 535 | 177 | 12 | read_image 只返回元数据（图片未入多模态），改用 SAM3 | read_image(0s) |
| 7 | 03:28:15 | 49,431 | 49,216 | 215 | 2468 | 62 | 决定并行分割 4 个物体（本轮思考 2.4k tok，62s） | segment(1s) segment(0s) segment(0s) segment(0s) |
| 8 | 03:28:57 | 52,627 | 48,704 | 3,923 | 1604 | 37 | SAM3 全部命中：沙拉酱 (-0.184,-0.080) 与相近瓶区分成功 | segment(2s) segment(0s) segment(0s) segment(0s) segment(0s) |
| 9 | 03:29:46 | 55,151 | 52,608 | 2,543 | 1833 | 43 | 交叉验证定位表：红挤瓶/高玻璃瓶/篮筐坐标确认 | back_project(0s) back_project(0s) back_project(0s) back_project(0s) |
| 10 | 03:30:40 | 57,408 | 55,104 | 2,304 | 2058 | 54 | 定位表完成，move_to 预定位至抓取点上方 | move_to(14s) |
| 11 | 03:31:12 | 60,124 | 48,768 | 11,356 | 125 | 18 | 预定位成功（Δ=8.8mm），决定腕部精修 | segment(1s) |
| 12 | 03:31:30 | 60,433 | 55,168 | 5,265 | 491 | 17 | 腕部精修通过（4mm 内），锁定最终抓取目标 | pi0_pick(48s) |
| 13 | 03:32:48 | 61,666 | 55,168 | 6,498 | 541 | 30 | pi0_pick 执行后 terminated=true（峰值抬升 0.196m，夹持 0.037） | segment(2s) |
| 14 | 03:33:25 | 62,395 | 55,168 | 7,227 | 1785 | 35 | 验证：瓶子已在篮筐区 (-0.005,0.264)，写审计 | write_text_file(0s) |
| 15 | 03:33:41 | 64,221 | 62,336 | 1,885 | 223 | 16 | 审计完成，finish(success) 收官 | finish(0s) |

可读出的微观规律：①上下文从 12.8k 逐轮涨到 64.2k（工具结果入历史），**缓存命中率随轮次上升**（首轮 0%，后半程 ~99%）；②LLM 耗时与思考量强相关（轮 7 思考 2.4k tok → 62s；轮 11 只 125 tok → 18s）；③真正的物理执行只占三行——move_to 14s、pi0_pick 48s（terminated=true），其余全是"大脑在想"。
